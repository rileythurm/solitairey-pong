...
pygame.display.set_caption("Pong Solitaire")
...
# TODO change make bubble grade mode so you don't have a score, and the bubbles don't have numbers, and there are no
# bad bubbles. And the grade isn't a simple percentage of bubbles caught, but it's combined with your amount of volleys so that you
# have to get a lot of volleys AND high bubble percentage. Maybe bubble caught % * volleys? So 100% success rate with only ten volleys
# would be a score of 10, but 100 volleys with 50% success would be 50.
...
if FLATULATION_ACTIVATION:
    BUBBLE_POP_SOUNDS.append(
        pygame.mixer.Sound(os.path.join("Assets", "Farts", "fart 1.wav"))
    )
...
def draw(
    ...
    # Developer info
    """
    width = SMALL_FONT.render(
        f"Paddle width: {str(round(paddleTop.width))}", 1, DEFAULT_DARK_COLOR
    )
    vel = SMALL_FONT.render(
        f"Velocity: {str(round(paddleTop.VEL))}", 1, DEFAULT_DARK_COLOR
    )
    odds_bad = SMALL_FONT.render(
        f"Odds bad: {str(odds_get_bad_bubble)}", 1, DEFAULT_DARK_COLOR
    )
    win.blit(width, (10, 20))
    win.blit(vel, (10, 60))
    win.blit(odds_bad, (10, 100))
    """
    # Score
